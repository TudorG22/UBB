...
def meniul() -> None:
    ...
    print("21) Rapoarte: primii 20% cei mai activi clienti (dupa numarul de imprumuturi)")
...
def afiseaza_carti_inchiriate_populare(cll) -> None:
    """Afiseaza cartile inchiriate cele mai des, in ordinea descrescatoare a imprumuturilor."""
    stats = cll.carti_inchiriate_statistici()
    if not stats:
        print("[nicio carte imprumutata]")
        return
    for book, count in stats:
        print(f"{book.get_CarteID()} '{book.get_CarteTitlu()}' de {book.get_CarteAutor()}  |  inchiriata de {count} ori")

def afiseaza_clienti_inchiriaza_sortati_dupa_nume(cll) -> None:
    """Afiseaza clientii care au imprumuturi, ordonati alfabetic."""
    clienti = cll.clienti_cu_imprumuturi_sortati_dupa_nume()
    if not clienti:
...
def afiseaza_clienti_inchiriaza_sortati_dupa_numar(cll) -> None:
    """Afiseaza clientii care au imprumuturi, ordonati descrescator dupa numarul lor."""
    clienti = cll.clienti_cu_imprumuturi_sortati_dupa_nr()
    if not clienti:
        print("[niciun client cu imprumuturi]")
        return
    for cl, books in clienti:
...
def afiseaza_clienti_top20_activi(cll) -> None:
    """Afiseaza primii 20% cei mai activi clienti (minim unul)."""
    clienti = cll.clienti_cei_mai_activi_top20()
    if not clienti:
        print("[niciun client cu imprumuturi]")
        return
    for cl, books in clienti:
...
def afiseaza_autori_inchiriati(cll) -> None:
    """Afiseaza autorii in ordinea descrescatoare a numarului de imprumuturi."""
    autori = cll.autori_inchiriati_statistici()
    if not autori:
        print("[nicio carte imprumutata]")
        return
