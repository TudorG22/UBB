...
class Console:
    """Interfata text care primeste serviciul ca dependinta."""
        ...
        while True:
            ...
            try:
                if opt == "1":
                    ...
                    self.service.replace_carte(cid, carte_noua)
                    ...
                    valid_id(clid)
                    self.service.delete_client(clid)
                    print("Client sters.")
                    ...
                    self.service.modify_client(clid, cl_nou)
                    print("Client modificat.")

                elif opt == "9":
                    afiseaza_clienti(self.service)
                    ...
                    valid_id(clid)
                    valid_id(cid)
                    self.service.adauga_imprumut(clid, cid)
                    ...
                    valid_id(clid)
                    valid_id(cid)
                    self.service.sterge_imprumut(clid, cid)
                    print("Cartea a fost stearsa din imprumuturi.")

                elif opt == "12":
                    clid = citeste_int("ID client: ")
                    valid_id(clid)
                    pair = self.service.gaseste_client(clid)
                    ...
                    valid_id(cid)
                    b = self.service.gaseste_carte(cid)
                    print(str(b))

                elif opt == "15":
                    clid = citeste_int("ID client: ")
                    valid_id(clid)
                    cl, books = self.service.gaseste_client(clid)
                    ...
                    for elt in genereaza_clienti(nr):
                        valid_id(elt.get_id())
                        self.service.add_client(elt)
                    print("Clienti generati cu succes.")

                elif opt == "18":
                    stats = self.service.top_carti_inchiriate()
                    if not stats:
                    ...
                    clienti = self.service.clienti_top20_activi()
                    if not clienti:
                        print("[niciun client cu imprumuturi]")
                    else:
                        for cl, books in clienti:
                    ...
                    autori = self.service.autori_inchiriati()
                    if not autori:
                        print("[nicio carte imprumutata]")
                    else:
                    ...
                    afiseaza_toate_imprumuturile(self.service)
...
def meniul() -> None:
    ...
    print("23) Afiseaza toate imprumuturile")
    print(" 0) Iesire")
...
def afiseaza_toate_imprumuturile(service: LibraryService) -> None:
    imprumuturi = service.imprumuturi_toate()
    if not imprumuturi:
        print("[fara imprumuturi]")
        return
...
def genereaza_clienti(nr: int, i=1, result=None) -> list:
    if result is None:
        result = []
    if i > nr:
        return result
    ...
    return genereaza_clienti(nr, i + 1, result)
